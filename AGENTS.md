...
- 手写 Go 代码必须以 `go.mod` 的 `go` 指令作为语言版本基线。新增或修改代码时，应主动识别并采用该版本已经稳定支持且适合当前场景的语言特性、内置能力、标准库 API 和惯用简化，不为低于该版本的工具链保留冗余兼容写法，也不得只机械套用本文列出的示例。以下仅为非穷举示例：复合字面量上下文能够推断元素类型时省略重复类型；使用 Go 1.27 及以上版本时，直接初始化嵌入结构体提升的字段，将 `Outer{Embedded: Embedded{Field: value}}` 简化为 `Outer{Field: value}`。生成代码不适用此约束。
